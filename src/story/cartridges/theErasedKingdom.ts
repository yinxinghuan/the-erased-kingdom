...
function build(locale: Locale): StoryCartridge {
  ...
  return {
    ...
    opening: {
      ...
      imagePrompt: 'cinematic grounded high fantasy, the player protagonist is a border courier who has just turned back after delivering an ordinary sealed letter, standing separately from young cartographer Mara on the village road of warm Apple Vale at dusk; Mara grips an old applewood ruler and urgently warns the courier while a blank road sign and distant orchard houses lose colour and dissolve into silent clean ivory absence; a confused wagon driver looks back from the road and the clerk who opened the letter is only a small background figure in the registry-house doorway; this is the instant BEFORE the player chooses what to save, with visible space between courier and Mara, shock and uncertainty, two readable focal subjects, natural anatomy, lived-in medieval clothing, restrained vermilion deletion traces, 4:5 portrait, central 58 percent safe composition; no handshake, no touching, no pulling, no rescue, no completed spell, no restored landmark, no royal seal being used, no battle, no knight, no text, no readable letters, no title, no logo, no UI, no frame',
      blocks: [
