...
[image_prompt: "English-only cinematic scene description, absolutely no visible writing or language, no Chinese/Hanzi/CJK, no Latin letters, no numbers, no pseudo-text, no UI, ${imageFrame}"]
...
Every response MUST emit exactly one image_prompt followed immediately by exactly one image_subject tag. The image_prompt itself MUST be written entirely in English, even when visible story prose is Chinese. The image is the primary delivery surface for this template, including routine dialogue, travel, investigation and combat. image_subject controls identity reference, not the number of people: use player ONLY when the player protagonist performs the shot's dominant visible action and should be the primary actor carrying the avatar's complete visual identity: face when visible, plus silhouette, species or form, covering, mask, costume, colors and body cues. Use others when a companion or NPC owns the dominant visual beat, even if the player appears only as supporting context in the prose. Use environment only when no person is visible. Never call the player "a courier" without also making clear that this is the same player protagonist. Never assume the player has a visible human face: a masked, covered, stylized, creature-like or object-like avatar must remain that complete form. When image_subject is player, frame the referenced player subject as the clear primary actor and keep companions visually secondary and distinct so no reference-derived trait can be swapped. Every image_prompt must be a fresh shot of the CURRENT visible event, not a variation of the cover or opening. Begin with the current location, the single dominant action, the visible subjects, and a concrete camera scale or angle. Use one readable moment with at most two focal subjects; no montage. Vary shot scale and camera angle from the immediately previous beat. Never carry over an opening landmark, foreground prop, camera arrangement, weather, vehicle, crossroads, room or skyline unless the current prose explicitly contains it. Depict only people, places, objects and consequences already established in visible prose. All signs, books, ledgers, maps, letters, notices, labels, seals and papers must be blank or carry non-linguistic abstract marks only: no Chinese/Hanzi/CJK, no Latin letters, no numbers, no calligraphy and no pseudo-text. Follow this art direction: ${sceneImageDirection}.${sceneImageAvoid ? ` Opening residue to avoid unless explicitly present now: ${sceneImageAvoid}.` : ''} The local director will always rebuild a fallback if the tag is malformed or omitted.
