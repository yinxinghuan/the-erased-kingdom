...
export const remoteAdapter: StoryAdapter = {
  ...
  async send(action, context, onProgress): Promise<AdapterResult> {
    ...
    const imageTextSafety = 'Write the image_prompt entirely in English. All signs, books, ledgers, maps, letters, notices, labels, seals and paper surfaces must be blank or use non-linguistic abstract marks only. No Chinese, Hanzi, CJK glyphs, Latin letters, numbers, calligraphy or pseudo-text.'
    const imageFreshness = `Make it a fresh shot of the CURRENT visible event: current location first, then one dominant action, visible subjects, and a concrete camera scale or angle. Use at most two focal subjects and no montage. Never carry over the cover/opening composition, landmarks, foreground props, weather, vehicle, crossroads, room or skyline unless the current prose explicitly contains them. If a courier is visible, explicitly identify whether that courier is the player protagonist; never substitute another person for the player's action. ${imageTextSafety}${sceneImageAvoid ? ` Opening residue to avoid unless explicitly present now: ${sceneImageAvoid}.` : ''}`
    const languageInstruction = context.locale === 'en'
      ? `\n\n[LANGUAGE AND FORMAT: Reply in English. Keep every protocol command tag and its syntax intact. End every response, including a genuine chapter checkpoint, with exactly three actions in this exact machine-readable form: [choices: "Action one"|"Action two"|"Action three"]. Button actions must match the decisions described in the prose. Inventory is transactional: whenever prose says the player obtains, stores, gives away, loses, discards, or consumes an item, emit the matching [inventory: action="add|remove" ...] command in the same response; merely seeing an item is not ownership. Record only durable, player-confirmed quest truths with [fact: id="stable-lowercase-id" value="true|false|number|short value"]. Reuse ids and never change a saved fact without a visible event. Use [true_ending: reason="..."] only after the player deliberately starts the final irreversible resolution; ordinary chapter pauses still use session_end. For a visually distinctive new place, discovery, relationship turn, major result, or checkpoint, propose one English scene prompt using [image_prompt: "cinematic visible scene, no text, no UI, ${imageFrame}"], immediately followed by [image_subject: "player|environment|others"]. image_subject controls identity reference: use player ONLY when the player performs the dominant visible action and should be the primary human; use others when a companion or NPC owns the dominant visual beat, even if the player appears only as supporting context; environment means no person. Depict only visible established facts and follow this art direction: ${sceneImageDirection}. ${imageFreshness} Skip routine conversation.]`
      : `\n\n[语言与格式要求：请用简体中文回复，并保持所有协议命令标签及语法不变。每次回复（包括真正的章节节点）都必须在结尾用这一机器可读格式给出恰好三个行动：[choices: "行动一"|"行动二"|"行动三"]。按钮行动必须与正文描述的决定一致。物品状态必须与叙事同步：正文只要明确说玩家获得、收下、装入、交出、失去、丢弃或消耗了物品，同一回复就必须输出对应的 [inventory: action="add|remove" ...]；只看见或检查物品不等于归玩家所有。只有玩家已经确认的长期任务真相、承诺、见证页、身份发现或地区结算才用 [fact: id="稳定小写-id" value="true|false|数字|短值"] 记录；必须复用 id，不能无事件改写已有事实。只有玩家主动开始最终不可逆处理时才使用 [true_ending: reason="..."]；普通章节暂停仍使用 session_end。遇到具有明显视觉价值的新地点、发现、关系转折、重大结果或阶段节点时，用 [image_prompt: "English cinematic visible scene, no text, no UI, ${imageFrame}"] 提议一张场景图，并紧接着输出 [image_subject: "player|environment|others"]。image_subject 用来控制头像身份参考：只有玩家执行画面中的主要动作、且应当成为主要人物时才用 player；同伴或 NPC 主导画面时用 others，即使玩家只作为陪衬出现在叙事中；environment 表示无人空镜。只画正文已经公开的事实，并遵循这一画风：${sceneImageDirection}。${imageFreshness} 普通对话不要提议。]`
    const dangerContract = dangerDirectiveContract(context.dangerDirective)
    const playerVisualContract = `PLAYER IMAGE CONTRACT: When image_subject is player, call the protagonist SUBJECT A in image_prompt. Describe SUBJECT A's action and props only. Never assign SUBJECT A a gender, age, ethnicity, species, face, hair, body type, anatomy, profession-shaped outfit or period clothing; the reference image alone owns those traits. Never use courier, traveler, knight, detective or another role noun as SUBJECT A's visual description. Give named NPCs their own separate identity.`
