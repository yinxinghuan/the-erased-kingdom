...
function milestoneReason(parsed: ParsedScene, dangerDirective?: DangerDirective): string {
...
function deriveCampaignFacts(facts: StorySave['facts']): StorySave['facts'] {
  ...
  commands.forEach((command, index) => {
    ...
    if (command.type === 'inventory') {
      ...
      if (existing) {
        ...
        next.inventory.push({
          id: command.itemId ?? `item-${next.scene}-${index}`, label: command.item, count: command.count, rarity: command.rarity,
