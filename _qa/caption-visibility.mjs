...
await page.waitForFunction(() => document.querySelector('.ct-result-story > button,.ct-turn-next,.st-composer input,.ct-stage__caption-page'))
...
await page.waitForFunction(() => document.querySelector('.ct-turn-next,.st-composer input,.ct-stage__caption-page'))
if (await entryResult.count()) await entryResult.click()
while (await page.locator('.ct-stage__caption-page').count()) await page.locator('.ct-stage__caption-page').click()
...
await page.locator('.st-quick-replies button').first().waitFor()
...
if (await page.getByRole('button', { name: /悬雨|屋檐/ }).count()) throw new Error('ungrounded generated choices were not filtered')
